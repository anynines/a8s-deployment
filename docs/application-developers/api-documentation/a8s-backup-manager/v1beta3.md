...
| `error` _string_ | Error message incase error occurred |
...
| `status` _string_ | Status status of the backup |
...
| `retentionPeriod` _[RetentionPeriod](#retentionperiod)_ | Period specifies the retention as duration |
...
| `type` _string_ | Type is the type of time unit for the retention period (hour, day, month, year) |
...
| Field | Description |
| --- | --- |
| `type` _ScheduleType_ | ScheduleType specifies the type of schedule to define date/time of next backup to run Currently we support Cron; more types to be implemented later |
